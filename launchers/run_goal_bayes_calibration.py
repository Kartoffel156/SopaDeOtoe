"""
run_goal_bayes_calibration.py — Sweep Bayesian calibration params
across all 16 graduated strategies.

Goal: goal-bayes-calibration.md
Runs: 16 strategies x 7 variations = 112

Usage:
    cd Patacon/
    python -m SopaDeOtoe.launchers.run_goal_bayes_calibration [--n-jobs 10]
"""
...
_DEFAULT_OUT = _SOPA_DIR / "results" / "goal_bayes_calibration"
...
def _build_all_configs(strategies):
    ...
    for strat_name, base_cfg in strategies:
        for var_name, overrides in VARIATIONS.items():
            ...
            cid = f"{strat_name}_bayes{var_name}_{ts}"
            cfg["config_id"] = cid
...
def main():
    ap = argparse.ArgumentParser(description="Bayesian calibration sweep")
