"""
run_goal_experimental_features.py — Sweep experimental meta-features
across all 16 graduated strategies.

Goal: goal-experimental-features.md
Runs: 16 strategies x 10 variations = 160

Usage:
    cd Patacon/
    python -m SopaDeOtoe.launchers.run_goal_experimental_features [--n-jobs 10]
"""
...
_DEFAULT_OUT = _SOPA_DIR / "results" / "goal_experimental_features"

# Each variation is a dict of dotted-key -> value overrides.
# For list values we use a special convention: the key targets the list directly.
VARIATIONS = {
    "A_variance_ratio": {
...
def _build_all_configs(strategies):
    ...
    for strat_name, base_cfg in strategies:
        for var_name, overrides in VARIATIONS.items():
            ...
            cid = f"{strat_name}_feat{var_name}_{ts}"
            cfg["config_id"] = cid
...
def main():
    ap = argparse.ArgumentParser(description="Experimental features sweep")
