"""
run_goal_feature_windows.py — Sweep feature window configurations
across all 16 graduated strategies.

Goal: goal-feature-windows.md
Runs: 16 strategies x 6 variations = 96

Usage:
    cd Patacon/
    python -m SopaDeOtoe.launchers.run_goal_feature_windows [--n-jobs 10]
"""
...
_DEFAULT_OUT = _SOPA_DIR / "results" / "goal_feature_windows"
...
VARIATIONS = {
    ...
    "F_entropy_bins5": {
        "v2.features.entropy_bins": 5,
    },
}


def _apply_dotted(cfg, key, value):
    """Apply a dotted key like 'v2.features.kyle_lambda_window' to nested dict."""
    parts = key.split(".")
    d = cfg
    for p in parts[:-1]:
        d = d.setdefault(p, {})
...
def _build_all_configs(strategies):
    ...
    for strat_name, base_cfg in strategies:
        for var_name, overrides in VARIATIONS.items():
            ...
            cid = f"{strat_name}_fw{var_name}_{ts}"
            cfg["config_id"] = cid
...
def main():
    ap = argparse.ArgumentParser(description="Feature windows sweep for all graduated strategies")
    ...
    print(f"\n{'Strategy':<50} " + " ".join(f"{v:>12}" for v in var_names))
    print("-" * (50 + 13 * len(var_names)))
    idx = 0
    for strat_name, _ in strategies:
        ...
        for _ in var_names:
            ...
            row += f"{s:+11.3f}  " if s is not None else "        ERR  "
            idx += 1
        print(row)
