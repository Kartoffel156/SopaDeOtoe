"""
run_goal_meta_features_importance.py — Ablation study: which meta-features
matter most for the meta-model across all graduated strategies.

Goal: goal-meta-features-importance.md
...
_DEFAULT_OUT = _SOPA_DIR / "results" / "goal_meta_features_importance"

# Shared base: all features ON (baseline reference)
_ALL_EXPERIMENTAL = [
...
VARIATIONS = {
    ...
    "B_no_variance_ratio": {
        "v2.features.meta_features.enabled_experimental_features": [
            "vol_accel", "autocorr_momentum", "kyle_entropy_ratio",
        ],
        "v2.features.meta_features.kronos_features.enabled": True,
        "v2.feature_prefilter.enabled": False,
    },
    "C_no_vol_autocorr": {
        "v2.features.meta_features.enabled_experimental_features": [
            "variance_ratio_5", "variance_ratio_20", "kyle_entropy_ratio",
        ],
        "v2.features.meta_features.kronos_features.enabled": True,
        "v2.feature_prefilter.enabled": False,
    },
    ...
    "I_kronos_only": {
        "v2.features.meta_features.enabled_experimental_features": [],
        "v2.features.meta_features.kronos_features.enabled": True,
        "v2.features.meta_features.disabled_meta_features": [
            "roll_spread", "parkinson_vol", "rolling_d_star",
            "vol_regime", "vol_of_vol", "autocorr_1", "rel_volume",
        ],
        "v2.meta_direction_feature": False,
        "v2.features.fracdiff": False,
        "v2.feature_prefilter.enabled": False,
...
def _build_all_configs(strategies):
    ...
    for strat_name, base_cfg in strategies:
        for var_name, overrides in VARIATIONS.items():
            ...
            cid = f"{strat_name}_imp{var_name}_{ts}"
            cfg["config_id"] = cid
...
def main():
    ap = argparse.ArgumentParser(description="Meta-features importance ablation")
    ...
    print(f"\n{'Strategy':<50} " + " ".join(f"{v:>20}" for v in var_names))
    print("-" * (50 + 21 * len(var_names)))
    idx = 0
    for strat_name, _ in strategies:
        ...
        for _ in var_names:
            ...
            row += f"{s:+19.3f}  " if s is not None else "                ERR  "
            idx += 1
        print(row)
