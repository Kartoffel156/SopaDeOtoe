"""
run_goal_meta_model_architecture.py — Sweep meta-model architecture
across all 16 graduated strategies.

Goal: goal-meta-model-architecture.md
Runs: 16 strategies x 7 variations = 112

Usage:
    cd Patacon/
    python -m SopaDeOtoe.launchers.run_goal_meta_model_architecture [--n-jobs 10]
"""
...
_DEFAULT_OUT = _SOPA_DIR / "results" / "goal_meta_model_architecture"
...
VARIATIONS = {
    ...
    "G_no_classweight": {
        "v2.meta_model.class_weight": None,
    },
}
...
def _build_all_configs(strategies):
    ...
    for strat_name, base_cfg in strategies:
        for var_name, overrides in VARIATIONS.items():
            ...
            cid = f"{strat_name}_arch{var_name}_{ts}"
            cfg["config_id"] = cid
...
def main():
    ap = argparse.ArgumentParser(description="Meta-model architecture sweep")
