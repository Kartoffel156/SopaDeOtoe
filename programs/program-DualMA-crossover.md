# Autoresearch — DualMovingAverageCrossover (Sharpe boost)

## Objetivo

Maximizar sharpe manteniendo n_trades >= 80 y total_return >= 1%.
...
Config graduado: `configs/graduated/DualMovingAverageCrossover_20260418_162631.yaml`
...
- total_return >= 0.01 (1%)
- max_drawdown >= -12%
- sharpe > 0.0974 (debe superar baseline)
...
- `short_period` (actual: 10, rango sugerido: 5-20)
- `long_period` (actual: 46, rango sugerido: 20-100)
...
- strategies/DualMovingAverageCrossover.py
- core/runner.py
- portfolio/runner.py
...
2. Ajustar ratio long/short (actualmente 4.6x, probar 3x, 5x, 6x)
3. Probar short_period mas rapido (5, 7) para entradas mas tempranas
4. Probar long_period mas largo (60, 80) para filtrar whipsaws
5. Ajustar pt_sl agresivamente — esta es la palanca mas importante con solo 2 params
6. Probar max_holding mas largo (42, 48) para dejar correr trends
7. Ajustar risk params: risk_fraction, risk_pct
