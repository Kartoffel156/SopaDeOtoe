# Autoresearch — H136 GoldenDeathCrossAsym (Sharpe boost)

## Objetivo

Maximizar sharpe manteniendo n_trades >= 45 y total_return >= 2%.
...
Config graduado: `configs/graduated/HypothesisH136GoldenDeathCrossAsym_20260411_010204.yaml`
...
- total_return >= 0.02 (2%)
- max_drawdown >= -10%
- sharpe > 0.2325 (debe superar baseline)
...
- `fast_period` (actual: 7, rango sugerido: 3-15)
- `slow_period` (actual: 16, rango sugerido: 10-50)
- `bear_thresh` (actual: 0.2624, rango sugerido: 0.10-0.50)
- `bull_thresh` (actual: 0.2183, rango sugerido: 0.10-0.50)
- `regime_window` (actual: 28, rango sugerido: 10-60)
...
- strategies/HypothesisH136GoldenDeathCrossAsym.py
- core/runner.py
- portfolio/runner.py
...
2. Ajustar fast/slow period ratio (la separacion entre MAs es clave)
3. Variar bear_thresh y bull_thresh independientemente (asimetria del filtro)
4. Probar regime_window mas corto (mas reactivo) y mas largo (mas estable)
5. Ajustar pt_sl para mejorar risk/reward por trade
6. Combinar direcciones ganadoras
