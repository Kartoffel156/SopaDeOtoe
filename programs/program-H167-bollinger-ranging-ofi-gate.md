# Autoresearch — H167 BollingerRangingOFIGate (Sharpe boost)

## Objetivo

Maximizar sharpe manteniendo n_trades >= 90 y total_return >= 3%.
...
Config graduado: `configs/graduated/HypothesisH167BollingerRangingOFIGate_20260418_162324.yaml`
...
- total_return >= 0.03 (3%)
- max_drawdown >= -10%
- sharpe > 0.1961 (debe superar baseline)
...
- `bb_period` (actual: 25, rango sugerido: 14-35)
- `bb_std` (actual: 2.2247, rango sugerido: 1.5-3.0)
- `ofi_window` (actual: 10, rango sugerido: 3-20)
- `ofi_thresh` (actual: 0.2135, rango sugerido: 0.05-0.5)
- `bw_pct` (actual: 37, rango sugerido: 20-60)
...
- strategies/HypothesisH167BollingerRangingOFIGate.py
- core/runner.py
- portfolio/runner.py
...
2. Subir ofi_thresh (0.3, 0.4) para filtrar OFI debil y mejorar calidad de senal
3. Bajar bw_pct (25, 30) para confirmar rango mas tight antes de entrar
4. Acortar ofi_window (5, 7) para OFI mas responsivo
5. Ajustar bb_std (1.8, 2.0) para bandas mas cerradas en ranging
6. Ajustar pt_sl: subir PT para capturar breakouts del rango
