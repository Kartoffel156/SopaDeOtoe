# Autoresearch — MultiTimeframeTrendSignal (Sharpe boost)

## Objetivo

Maximizar sharpe manteniendo n_trades >= 110 y total_return >= 0.2%.
...
Config graduado: `configs/graduated/MultiTimeframeTrendSignal_20260419_134237.yaml`

Nota: esta estrategia tiene el sharpe y return mas bajos del portfolio.
El objetivo principal es subir sharpe significativamente.

## Restricciones duras

- n_trades >= 110
- total_return >= 0.002 (0.2%)
- max_drawdown >= -10%
- sharpe > 0.0686 (debe superar baseline)
...
- `short_window` (actual: 26, rango sugerido: 10-40)
- `mid_window` (actual: 52, rango sugerido: 30-80)
- `long_window` (actual: 236, rango sugerido: 100-400)
- `vol_window` (actual: 58, rango sugerido: 20-100)
...
- strategies/MultiTimeframeTrendSignal.py
- core/runner.py
- portfolio/runner.py
...
2. Acortar long_window (150, 180) — 236 puede ser demasiado lento para BTC
3. Ajustar ratios entre ventanas: short/mid ~0.5x, mid/long ~0.3x
4. Sweep vol_window: 30, 40, 50, 70 (filtrado de volatilidad)
5. Probar short_window mas corto (15, 20) para senales mas rapidas
6. Ajustar pt_sl agresivamente — con sharpe tan bajo, pt_sl es palanca critica
7. Probar max_holding mas largo (42, 48) para capturar trends completos
